...
from etc_workflow.utils import _connect_mongo_products_collection, _get_products_by_tile_and_date, _get_sentinel, _safe_minio_execute, _get_minio
...
def _get_available_products(tiles: dict, mongo_collection: Collection,  seasons: dict):
    ...
    for tile in tiles:
        for season in seasons:
            ...
            product_metadata_cursor = _get_products_by_tile_and_date(tile, mongo_collection, start_date, end_date, 101)
...
def compute_cloud_coverage(countries: List[str]):
    ...
    seasons = {'spring' : (datetime(2021, 3, 1), datetime(2021, 3, 31)),
               'summer' : ( datetime(2021, 6, 1), datetime(2021, 6, 30)),
               'autumn' : (datetime(2021, 11, 1), datetime(2021, 11, 30))}

    tiles_by_country = _get_available_products(mongo_collection=mongo, tiles=tiles, seasons=seasons)
