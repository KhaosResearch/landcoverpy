...
def postprocess_dataset_forest(df: pd.DataFrame):
    ...
    Postprocess the dataset for forest classification:
        ...
        - Remove pixels without `forest_type` value and "No arbolado" values and values not in [0,1,2,3,4,5,6]
        - Shuffle the whole dataset
    """

    df_all_forests = df[df["class"].isin(["mixedForest", "treePlantation", "riparianForest", "closedForest", "openForest"])]
    df_all_forests = df_all_forests[~((df_all_forests["forest_type"].isna()) | (df_all_forests["forest_type"] == "No arbolado") | (~df_all_forests["forest_type"].isin(["0","1","2","3","4","5","6"])))]
