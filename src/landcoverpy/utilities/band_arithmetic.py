...
def true_color(r: Path, g: Path, b: Path, *, output: Path= None) -> np.ndarray:
    ...
    if output:
        kwargs.update(driver="GTiff", dtype=rasterio.float32, nodata=0, count=3, compress="deflate", predictor=3)
...
def moisture(b8a: Path, b11: Path, *, output: Path= None) -> np.ndarray:
    ...
    * https://docs.digitalearthafrica.org/en/latest/data_specs/Sentinel-2_Level-2A_specs.html
    ...
    if output:
        kwargs.update(driver="GTiff", dtype=rasterio.float32, nodata=np.nan, count=1, compress="deflate", predictor=3)
