...
def workflow(client: Client, predict: bool, tiles_to_predict: List[str] = None, columns_predict: List[str] = None):
...
def process_tile(tile, predict, polygons_in_tile, columns_predict=None):
    ...
    for dem_name in dems_raster_names:
        ...
        if (not predict) or (predict and dem_name in columns_predict):
            ...
            if not predict:
                crop_mask, label_lon_lat = mask_polygons_by_tile(polygons_in_tile, kwargs)
            if predict:
